...
function Admin_RestrictionTags() {
...
useEffect(() => {
  const getRestrictionTags = async () => {
    try {
      const response = await fetch(
        `http://localhost:8000/restaurants/restrictiontags/`,
        {
           method: "GET",
        headers: {
    ...
    if (newTag) {
      try {
        ...
        if (response.ok) {
          ...
          setRestrictionTags([...restrictionTags, data]);
...
const handleEditTag = (tag) => {
  ...
  const submitEdit = async () => {
    if (editedTag && editTag) {
      try {
        const response = await fetch(
          `http://localhost:8000/restaurants/restrictiontags/${editTag.id}/`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
...
const handleDeleteTag = async (tagId) => {
  try {
    const response = await fetch(
      `http://localhost:8000/restaurants/restrictiontags/${tagId}/`,
      {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${access}`,
    ...
    if (response.ok) {
      ...
      const updatedTags = restrictionTags.filter((tag) => tag.id !== tagId);
      setRestrictionTags(updatedTags);
    } else {
      console.error("API Error:", response.status);
  ...
  <View style={styles.container}>
    ...
    <View style={styles.headerContainer}>
      <Text style={styles.headerText}>Tag Management - Restriction Tags</Text>
      <View style={styles.descriptionContainer}>
        <Text style={styles.tagsDescription}>
          This section allows you to manage restriction tags. You can add new tags, delete existing ones, and edit the titles of the tags.
        </Text>
      </View>
...
<View style={styles.tagsBox}>
  <Text style={styles.tagsHeader}>Restriciton Tags</Text>
...
export default Admin_RestrictionTags;
