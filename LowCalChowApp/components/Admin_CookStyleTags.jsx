...
useEffect(() => {
  const getCookstyleTags = async () => {
    try {
      const response = await fetch(
        `http://localhost:8000/restaurants/cookstyletags/`,
        {
           method: "GET",
        headers: {
    ...
    if (newTag) {
      try {
        ...
        if (response.ok) {
          ...
          setCookstyleTags([...cookstyleTags, data]);
...
const handleEditTag = (tag) => {
  ...
  const submitEdit = async () => {
    if (editedTag && editTag) {
      try {
        const response = await fetch(
          `http://localhost:8000/restaurants/cookstyletags/${editTag.id}/`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
...
const handleDeleteTag = async (tagId) => {
  try {
    const response = await fetch(
      `http://localhost:8000/restaurants/cookstyletags/${tagId}/`,
      {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${access}`,
    ...
    if (response.ok) {
      ...
      const updatedTags = cookstyleTags.filter((tag) => tag.id !== tagId);
      setCookstyleTags(updatedTags);
    } else {
      console.error("API Error:", response.status);
  ...
  <View style={styles.container}>
    ...
    <View style={styles.headerContainer}>
      <Text style={styles.headerText}>Tag Management - Cook Style Tags</Text>
      <View style={styles.descriptionContainer}>
        <Text style={styles.tagsDescription}>
          This section allows you to manage Cook Style tags. You can add new tags, delete existing ones, and edit the titles of the tags.
        </Text>
      </View>
...
<View style={styles.tagsBox}>
  <Text style={styles.tagsHeader}>Cook Style Tags</Text>
...
export default Admin_CookStyleTags;
