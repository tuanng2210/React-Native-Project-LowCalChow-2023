  ...
  const getingredientTags = async () => {
    try {
      const response = await fetch(
        `http://localhost:8000/restaurants/ingredienttags/`,
        {
           method: "GET",
        headers: {
    ...
    if (newTag) {
      try {
        ...
        if (response.ok) {
          ...
          setingredientTags([...ingredientTags, data]);
...
const handleEditTag = (tag) => {
  ...
  const submitEdit = async () => {
    if (editedTag && editTag) {
      try {
        const response = await fetch(
          `http://localhost:8000/restaurants/ingredienttags/${editTag.id}/`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
...
const handleDeleteTag = async (tagId) => {
  try {
    const response = await fetch(
      `http://localhost:8000/restaurants/ingredienttags/${tagId}/`,
      {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${access}`,
    ...
    if (response.ok) {
      ...
      const updatedTags = ingredientTags.filter((tag) => tag.id !== tagId);
      setingredientTags(updatedTags);
    } else {
      console.error("API Error:", response.status);
  ...
  <View style={styles.container}>
    ...
    <View style={styles.headerContainer}>
      <Text style={styles.headerText}>Tag Management - Ingredient Tags</Text>
      <View style={styles.descriptionContainer}>
        <Text style={styles.tagsDescription}>
          This section allows you to manage ingredient tags. You can add new tags, delete existing ones, and edit the titles of the tags.
        </Text>
      </View>
...
<View style={styles.tagsBox}>
  <Text style={styles.tagsHeader}>Ingredient Tags</Text>
...
export default Admin_IngredientTags;
